...
import {color,KEYS,inferNewNumbers,matches,TITLES} from './strategies.js';
...
async function processSpin(tableId,num,source='api'){
  ...
  const {rows:profiles}=await pool.query(`SELECT tp.*,p.table_id AS selected_table
    FROM telegram_profiles tp JOIN preferences p ON p.user_id=tp.user_id
...
export async function pollOnce(){
  ...
  try{
    ...
    live.online=false; live.error=String(e.message).slice(0,250); console.error('[coleta]',live.error);
  }finally{busy=false;}
}
let sendBusy=false;
export async function deliverOne(){
  ...
  try{
    const {rows}=await pool.query(`SELECT o.*,tp.label,tp.bot_token_cipher,tp.chat_id_cipher,tp.thread_id,tp.send_enabled,tp.flags,tp.armed_after_id,p.table_id AS selected_table
      FROM outbox o JOIN telegram_profiles tp ON tp.id=o.profile_id
